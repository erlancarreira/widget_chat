...
- **Checagem do número sem bloquear o lead (capture-first):** `startChat` consulta a Evolution
  (`POST /chat/whatsappNumbers/{instance}`, Baileys onWhatsApp) apenas para ROTEAR. Número sem
  WhatsApp → o grupo nasce só com a plataforma, o atendimento/histórico seguem pelo site e uma
  **mensagem de sistema** marca o canal como inalcançável para retorno por outro contato. O lead
  nunca é barrado por validação de canal; checador indisponível → segue sem validar (a checagem
  nunca é ponto único de falha do atendimento).
